...
from torch_geometric.datasets import Planetoid, Amazon, Coauthor, WikiCS, WikipediaNetwork, WebKB, Actor, \
    AttributedGraphDataset, TUDataset, CitationFull, HeterophilousGraphDataset, OGB_MAG
...
class Dataset:
    ...
    def prepare_data(self, ds_name, feat_norm, from_npz):
        ...
        elif ds_name in ['ogbn-arxiv', 'ogbn-products', 'ogbn-papers100M', 'ogbn-proteins']:
            self.data_raw = ogb_load_dataset(ds_name, path=self.path)
            self.g = self.data_raw[0]
            self.feats = self.g.x
            self.n_nodes = self.feats.shape[0]
            self.dim_feats = self.feats.shape[1]
            self.labels = self.g.y.squeeze()
            self.adj = torch.sparse_coo_tensor(self.g.edge_index, torch.ones(self.g.edge_index.shape[1]),
                                               [self.n_nodes, self.n_nodes])
            self.n_edges = self.g.num_edges / 2
            self.n_classes = self.data_raw.num_classes
        ...
        self.homophily = get_homophily(self.labels, self.adj, type='edge', fill=None)
        ...
        print("""----Data statistics------'
                ...
                #Ave_degree %.2f
                #Homophily %.2f""" %
              (self.name, self.n_nodes, self.n_edges, self.n_feats, self.n_classes, self.ave_degree, self.homophily))
    ...
    def split_data(self, verbose=True):
        ...
        if self.split_type == 'default':
            if self.name in ['ogbn-arxiv', 'ogbn-products', 'ogbn-papers100M']:
                split_idx = self.data_raw.get_idx_split()
                train_indices = split_idx["train"].squeeze().numpy()
                val_indices = split_idx["valid"].squeeze().numpy()
                test_indices = split_idx["test"].squeeze().numpy()
            elif not hasattr(self.g, 'train_mask'):
...
def pyg_load_dataset(name, path='./data/'):
    ...
    elif name in ['blogcatalog', 'flickr', 'tweibo']:
