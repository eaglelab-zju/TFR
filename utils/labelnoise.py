...
def add_instance_dependent_label_noise(n, feature, labels, num_classes, feature_size, norm_std, seed):
...
def label_process(labels, features, n_classes, noise_type='uniform', noise_rate=0, random_seed=5, debug=True):
    ...
    if noise_rate > 0.0:
        if noise_type == 'clean':
            ...
            cp = np.eye(n_classes)
            print("Invalid noise type: " + noise_type)
            exit(1)
    ...
    if noise_rate > 0.0:
        if cp is not None:
            ...
            noisy_labels = add_instance_dependent_label_noise(noise_rate, features, labels.cpu().numpy(), n_classes, features.shape[1], 0.1, random_seed)
