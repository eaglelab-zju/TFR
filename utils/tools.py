...
def get_edge_homophily_sparse(label, adj):
    '''
    Calculate the edge homophily of a graph.
...
def get_homophily(label, adj, type='node', fill=None):
    ...
    if type == 'node':
        return get_node_homophily(label, adj.to_dense())
    else:
        return get_edge_homophily_sparse(label, adj)
...
def tsne_visualization(representation, labels, title):
    '''
    Visualization of the representation using t-SNE.
    Parameters
    ----------
    representation: torch.Tensor
        Node representation to be visualized.
    labels: torch.Tensor
        Nodes will be colored by labels.
    title: string
        Figure title.
    ...
    X_embedded = TSNE(n_components=2, learning_rate='auto',
                      init='random', perplexity=30, n_jobs=20, early_exaggeration=60).fit_transform(representation)
    ...
    for i in range(n_classes):
        selected_idx = np.where(labels == i)[0]
        plt.scatter(X_embedded[selected_idx, 0], X_embedded[selected_idx, 1], c=color_list[i], marker='o', s=5, label='Class {}'.format(i))
    plt.legend()
    plt.title(title)
    plt.savefig('./data_eval/' + title + '.png', format='png')
    plt.show()