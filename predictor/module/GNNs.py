...
class GAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, n_layers=3, n_heads=1, dropout=0.5):
        ...
        self.convs = nn.ModuleList()
        if n_layers == 1:
            self.convs.append(GATConv(in_channels, out_channels, heads=n_heads, dropout=dropout, concat=False))
        else:
            self.convs.append(GATConv(in_channels, hidden_channels, heads=n_heads, dropout=dropout))
            for _ in range(n_layers - 2):
                self.convs.append(GATConv(hidden_channels, hidden_channels, heads=n_heads, dropout=dropout))
            self.convs.append(GATConv(hidden_channels, out_channels, heads=n_heads, dropout=dropout, concat=False))
